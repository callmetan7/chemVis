...
class MoleculeBuilder {
  ...
  calculateValence(atom) {
    ...
    if (
      ["S", "P", "Cl", "Br", "I"].includes(atom.symbol) &&
      atom.bonds.length > 4
    ) {
  ...
  handleRemainingBonds() {
    ...
    this.atoms.forEach((atom1) => {
      this.atoms.forEach((atom2) => {
        if (
          atom1.id < atom2.id && // Prevent duplicate checks
    ...
    this.atoms.forEach((atom1) => {
      atom1.bonds.forEach((atom2Id, i) => {
        ...
        if (!printed.has(bondKey)) {
          ...
          const bondType = ["-", "=", "≡"][atom1.bondOrders[i] - 1] || "-";
          console.log(
            `${atom1.symbol}${bondType}${atom2.symbol} (order: ${atom1.bondOrders[i]})`
          );
          printed.add(bondKey);
        }
      });
  ...
  visualize(containerId = 'body') {
        new MoleculeViewer(this, containerId);
    }
}
